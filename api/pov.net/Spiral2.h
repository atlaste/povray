...
namespace povray
{
	namespace Materials
	{
		namespace Patterns
		{
			public ref class Spiral2 : public Pattern, public ITargetType<Pigment^>, public ITargetType<Normal^>, public ITargetType<Texture^>
			{
			internal:
				...
				void Render(Context^ context, PatternType* container)
				{
					...
					auto pat = new pov::Spiral2Pattern();
			...
			public:
				Spiral2(float numberOfArms) :
					NumberOfArms(numberOfArms),
					BumpSize(1)
				{}

				Spiral2(float numberOfArms, float bumpSize) :
					NumberOfArms(numberOfArms),
					BumpSize(bumpSize)
				{}
